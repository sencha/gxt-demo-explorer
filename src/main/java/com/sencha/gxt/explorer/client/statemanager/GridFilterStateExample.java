...
@Detail(
  name = "Grid Filter State",
  category = "State Manager",
  icon = "filtergrid",
  classes = { Stock.class, StockProperties.class, TestData.class },
  maxHeight = GridFilterStateExample.MAX_HEIGHT,
  maxWidth = GridFilterStateExample.MAX_WIDTH,
  minHeight = GridFilterStateExample.MIN_HEIGHT,
  minWidth = GridFilterStateExample.MIN_WIDTH)
public class GridFilterStateExample implements IsWidget, EntryPoint {
  ...
  private GridFilterStateHandler<Stock> gridState;
  ...
  public Widget asWidget() {
    if (panel == null) {
      ...
      panel.setHeading("Filter Grid State Example");
      panel.add(grid);

      // 2. Stage manager, load the previous state
      gridState = new GridFilterStateHandler<Stock>(grid, filters);
      grid.setStateful(true);
      grid.setStateId("filterGridStateExample");
    }

    // 3. Load the previous state.
    gridState.loadState();
